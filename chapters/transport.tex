...
\paragraph{The Fokker Planck equation} In the soft momentum transfer $q = p'-p$ limit  $|q| \ll |p|$, the change in distribution function can be expanded to the second terms in the $q$, and the linearized Boltzmann equation reduces to the Fokker-Planck type of equation,
\begin{eqnarray}
\frac{df}{dt} &=& - \int \left\{
...
\end{eqnarray}
Where the vector function $A$ and tensor function $B$ are the first and second moments of the transition probability,
\begin{eqnarray}
...
The $A$ and $B$ function has to satisfy certain symmetry, as the only special direction after averaging over medium effects is the direction of motion.
Therefore,  $\vec{A} = \eta_D \vec{p}$ and it defines the drag coefficient  $\eta_D$; while the tensor $B$ can be decomposed into a transverse part and a longitudinal part, with the respective transport coefficients $\kappa$ and $\kappa_L$ 
\begin{eqnarray}
...
The diffusion coefficient are directly related to the one in the Fokker Planck equation $\Kpara = \hat{q}_L$, and $\Kperp = \hat{q}/2$.
...
In the post-step scheme, this relation is,
\begin{eqnarray}
...
However, the formation time can be sufficiently long for collinear branching that it greatly exceeds the elastic collision mean-free path.
This happens for small transverse momentum of the splitting that $k_\perp^2, p_\perp^2 < g^2x(1-x)ET$.
In this region of phase space, the average number of collision during the radiation becomes a relatively large number $N = \tau_f/\lambda >1$, so
the picture of independent radiation induced from independent scattering centers breaks down.
Zakharov and later by [BDMPS] showed that these multiple scatterings should be resumed, and the leading order resumed radiation probability is
\begin{eqnarray}
\nonumber
...
Finally, the interaction $\Gamma_3(\mathbf{b})$ encodes the transverse broadening of the three body system $a\rightarrow b+c$.
It has three pieces of two-body contributions $\Gamma_2(\mathbf{b})$.
The vacuum pieces $G_0$ is subtracted from $G$ so that this formula only computes the medium-induced radiation.
...
For example, in a weakly coupled theory, the leading order contribution is
\begin{eqnarray}
...
Another approach which studies the solution assuming soft interaction (small-$q$) and approximate the collision kernel by a harmonic oscillator one $\Gamma_2(b) \approx \frac{1}{4}\hat{q}b^2$.
...
Taking the residue potential $\Gamma(b) - \frac{1}{4}\hat{q}b^2$ as a perturbation, improvements at the next-to-leading-log level has also been investigated in [P Arnold \& Yacine].
...
This form is known as the Gunion-Bertsch approximation of inelastic $2\rightarrow 3$ scattering, whose improved form has been employed in existing full Boltzmann simulation of partonic transport equation [cite BAMPS].
...
Compared to the na\"ive ``incoherent expectation'', the actual radiation rate is reduced by a factor of $\lambda_{\textrm{el}}/\langle \tau_f \rangle$ on average. 
...
This issue has been studied at the next-to-leading-log (NLL) level by treating the large-$q$ part of the collision kernel as a perturbation to this approximation, authors of [P Arnold] and more recently in [Yacine] had found that reasonable choice of $Q_0$ is the order of $k_\perp^2$ itself.
...
In the next section, we shall also make use of this NLL solution to validate the performance of the transport simulation.
...
We follow the approach described in [arXiv:1006.2379] to solve the propagator $G$ in the momentum space.
...
Then, we compare the simulation of the ``a modified Boltzmann transport" to theoretical calculations introduced in the previous sections for different parton energy, coupling constants, and medium temperatures that are relevant for phenomenology applications.
...
We need to address what the $Q_0$ scale is in the Botlzmann simulation and how to improve that.
...
To evaluate the accuracy of this procedure, we will also compare the radiation spectrum off the heavy quark to the exact solution for heavy quark.
...
From \cite{Arnold:2008zu}, the elastic collision couplings are evaluated at $\hat{t}$-channel momentum transfer $q_\perp^2$.
...
For the splitting vertex, on the theoretical side \cite{Arnold:2008zu} it is included as a change of the $\hat{q}_3$ in the NLL formula to 
\begin{eqnarray}
...
This approach is employed in transport models like MARTINI and TEQULIA [,].
Our approach, on the other hand, takes into account the changing of the medium temperature (and also flow velocity).
...
Also as mentioned before, in the absent ot the pertrubative tail in the collision kernel, $b=0.75$ is used without the logarithmic correcting factor in equation \ref{eq:NLL-b}.
In additional, we will not try to make a direct comparison of the spectra (left of figure \ref{fig:Bjorken-BDMPS}), but focusing more on the ratio between the expanding calculation/simulation over the static calculation/simulation instead.
...
In figure \ref{mass.png} for the bottom quark case, the horizontal axis is the path-length, and the vertical axis is the ratio.
...
I had used the previous approach in my earlier studies, but it is the problems I encountered in this method that later motivates the development of the ``modified Boltzmann transport'' method. 
...
This divergence is not a problem for computing more physical quantities such as energy loss, as it will be balanced by the gluon absorption processes.
However, to use the radiation rate formulation, infrared cut-off has to be introduced $x_c$. 
...
However, the ambiguity rises from the way people handle multiple emissions using this formula.
...
Here we would like to discuss another method in dealing with multiple emission using the high twist formula in a time evolution manner [Shanshan].
...
This is another approach we found in the literature [PCM LPM].
...
The two-body scatterings between quarks, anti-quarks and gluons are standard and we quote the results from existing references [].
...
We work under the collinear approximation $k_\perp^2, q_\perp^2 \ll x(1-x) \hat{s}$, and $x = k^+/\sqrt{s} = k_\perp e^y_k /\sqrt{s}$, and only consider the $\hat{t}$-channel momentum exchange between the medium and the hard partons.
...
\end{eqnarray}
We follow the derivation in [BAMPS-Gunion-Bertsch] to choose the light-cone gauge with light-like vector $n = (0, 1, 0)$, and the gauge fixing condition $n\cdot A =0$ eliminates the ``+" component in the gluon (with momentum $p$) polarization vector, and is obtained by applying the transverse condition $\epsilon \cdot p = 0$ (up to a higher order correction to its normalization)
\begin{eqnarray}
...
To combine the results that is obtained in different regions of phase space ($y_k > 0$ and $y_k < 0$), we follow [BAMPS] and defines,
\begin{eqnarray}
