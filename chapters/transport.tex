\chapter{Partonic transport model}
In this section we describe the transport equation for high energy partons.

We first start with the transport equation, regard less of the fact that the radiation process can be coherent for a long distance.
We will study how to improve the Boltzmann type simulation to include such coherence effect.
...
\begin{eqnarray}
R &\propto& 2g^2 \int d k^2 \int d q^2 \mathcal{A}(q_\perp^2) \sum_{\pm}
\frac{C_A}{2} \frac{\vec{k}}{\vec{k}^2}\cdot\left[\frac{\vec{k}}{\vec{k}^2}-\frac{\vec{k}\pm\vec{q}}{(\vec{k}\pm\vec{q})^2}\right] \\
&=& g^2 C_A \int d k^2 \int d q^2 \mathcal{A}(q_\perp^2) \cdot\left[\frac{\vec{k}}{\vec{k}^2}-\frac{(\vec{k}-\vec{q})}{(\vec{k}-\vec{q})^2}\right]^2 
\end{eqnarray}
where in the last step, the $k$ integration of the term with the ``$+$" sign has been shifted to an integration over $k-q$ to render the expression into the completing square form.
...
Compared to the ``incoherent expectation", the actual radiation rate is reduced by a factor of,

{\bf The deep-LPM region (next-to-leading-log level)}:
The previously introduced leading (log) result has both a charming simplicity and a clear physical interpretation explaining what happens in in the deep-LPM region $\ln(N) \sim \ln(xE/T) \gg 1$
Together with the Bethe-Heitler (incoherent) limit at $xE \lesssim T$, one can already build a pretty good understanding of 
This cut-off scale is generally unknown and bring an uncertainty in this approximation. 
However, treating the large-$Q$ part of the collision kernel as a perturbation to this approximation, authors of [] and [] has found that the optimal value for $Q_0$ to be the order of $k_\perp^2$ itself.
...
\section{A modified Boltzmann approach to include the LPM effect}
