...
In this section, we describe the Monte Carlo model in detail. 
...
\section{A Monte Carlo solver of diffusion plus scattering}
...
\end{eqnarray}
The two operations ``diffusion'' and ``scatterings''  are treated by very different simulation technique and the results of the order of operation also matters.
However, with a small enough time step, the order of operation only changes the distribution function at $O(\Delta t^2)$.
...
Our Monte Carlo method starts from representing the distribution function by delta-function like particle states $\delta^{3}(x-x'- v(t-t')) \delta^3(p-p')$. 
...
The details of the Monte-Carlo solver are explained in this section.
...
\end{eqnarray}
The Langevin equation governs the evolution of an ensemble of test particles whose distribution is the solution of the Fokker-Plank equation.
\begin{eqnarray}
...
\section{Heavy quark energy loss and thermalization in static medium}
\begin{figure}
